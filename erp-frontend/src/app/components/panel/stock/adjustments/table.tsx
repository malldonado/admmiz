...
const TableHeader: FC = () => (
  <thead className="bg-gray-300 dark:bg-gray-200">
    <tr>
      {["Código", "Movimentação", "Frete", "Valor Total", ""].map((header) => (
        <th
          key={header}
          scope="col"
...
const TableRow: FC<User> = ({ id, movement, freight, totalValue }) => (
  <tr>
    <td className="px-4 py-4 text-sm text-black whitespace-nowrap">{id}</td>
    <td className="px-4 py-4 text-sm text-black whitespace-nowrap">{movement}</td>
    <td className="px-4 py-4 text-sm text-black whitespace-nowrap">{freight}</td>
    <td className="px-4 py-4 text-sm text-black whitespace-nowrap">{totalValue}</td>
    <td className="px-4 py-4 text-sm text-black whitespace-nowrap flex justify-end items-center">
      <NextLink href="/pages/stock/adjustments/edit">
        <FiEdit
