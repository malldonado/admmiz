...
const TableHeader: FC = () => (
  <thead className="bg-gray-300 dark:bg-gray-200">
    <tr>
      {["Nome", "Padrão de leitura", ""].map((header) => (
        <th
          key={header}
          scope="col"
...
const TableRow: FC<User> = ({ name, readingPattern }) => (
  <tr>
    <td className="px-4 py-4 text-sm text-black whitespace-nowrap">{name}</td>
    <td className="px-4 py-4 text-sm text-black whitespace-nowrap">{readingPattern}</td>
    <td className="px-4 py-4 text-sm text-black whitespace-nowrap flex justify-end items-center">
      <NextLink href="/pages/stock/movement/edit">
        <FiEdit
