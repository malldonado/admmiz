...
const TableHeader: FC = () => (
  <thead className="bg-gray-300 dark:bg-gray-200">
    <tr>
      {["Nome", "Tipo", "Exibir na impressão", "Cadastrado em", ""].map((header) => (
        <th
          key={header}
          scope="col"
...
const TableRow: FC<User> = ({ name, type, showPrint, register }) => (
  <tr>
    <td className="px-4 py-4 text-sm text-black whitespace-nowrap">{name}</td>
    <td className="px-4 py-4 text-sm text-black whitespace-nowrap">{type}</td>
    <td className="px-4 py-4 text-sm text-black whitespace-nowrap">{showPrint}</td>
    <td className="px-4 py-4 text-sm text-black whitespace-nowrap">{register.getDate()}</td>
    <td className="px-4 py-4 text-sm text-black whitespace-nowrap flex justify-end items-center">
      <NextLink href="/pages/sales/extra-fields/edit">
        <FiEdit
